# AI Resume Regenerator - Production Version 2026.02.04 17:45
import os
import tempfile
import re
...
4. Professional Title should be JUST the job title (e.g. "Senior Java Developer").
5. **Total Experience**: Look for phrases like "3+ years", "5 years experience". Calculate if necessary from dates.
6. **Notice Period**: Look for "Notice Period", "Availability", "can join in". If not found, leave empty.
...
def create_resume_pdf(data, path):
    ...
    if data.get("Total Experience"): 
        parts.append(data["Total Experience"])
    
    if data.get("Notice Period"):
        parts.append(f"Notice Period: {data['Notice Period']}")
